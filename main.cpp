...
//adds two given matrices, returns the result and gives report about error existence
{
    if ((firstArg->width!=secondArg->width)||(firstArg->height!=secondArg->height)){
        std::cout<<"Dimention mismatch. Addition."<<std::endl;
        *errors = true;
...
{
    if ((firstArg->width!=secondArg->width)||(firstArg->height!=secondArg->height)){
        std::cout<<"Dimention mismatch. Subtraction."<<std::endl;
        *errors = true;
        cMatrix result = cMatrix(1, 1);
        return result;
    }
...
//multiplies two given matrices, returns the result and gives report about error existence
{
    if (firstArg->width!=secondArg->height){
        std::cout<<"Dimention mismatch. Multiplication."<<std::endl;
        *errors = true;
        cMatrix result = cMatrix(1, 1);
        return result;
    }
...
int main()
{
    ...
    cin>>w;
    if(!cin.good()){
        cout<<"Wrong input. End of programme."<<endl;
        return 0;
    }
    ...
    cin>>h;
    if(!cin.good()){
        cout<<"Wrong input. End of programme."<<endl;//here a valid integer followed by some characters is read as correct
        return 0;
    }
